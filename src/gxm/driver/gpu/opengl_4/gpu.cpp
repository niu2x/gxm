...
void gpu::buffer_resize(const core::rid &p_rid, size_t bytes) {
    ...
    if (my_buffer) {
        glNamedBufferData(
            my_buffer->name, bytes, nullptr, GL_DYNAMIC_DRAW);
    }
}
...
void gpu::shader_compile(const core::rid &p_rid, const char *src) {
    ...
    if (!my_shader->ok) {
        ...
        GXM_LOG_E(
            std::string("") + "shader compile failed: " + log.data());
    }
}
...
void gpu::program_attach_shader(
    const rid &p_rid,
    const rid &p_shader) {
...
void gpu::program_link(const rid &p_rid) {
    ...
    if (!my_program->ok) {
        ...
        GXM_LOG_E(
            std::string("") + "program link failed: " + log.data());
    }
}

...
static int module_newindex_event(lua_State *L) {
    ...
    if (lua_istable(L, -1)) {
        ...
        if (lua_iscfunction(L, -1)) {
            lua_pushvalue(
                L, 1); /* only to be compatible with non-static vars */
            ...
            if (lua_isnumber(L, 2))       /* check if key is a numeric value */
            {
                ...
                if (lua_istable(L, -1)) {
                    ...
                    if (lua_iscfunction(L, -1)) {
                        ...
                        /* deal with array: create table to be returned and
                         * cache it in peer */
                        ...
                        lua_setmetatable(
                            L, -2); /* set stored value as metatable */
                        lua_pushvalue(
                            L, -1); /* stack: obj key met table table */
                        lua_pushvalue(
                            L, 2); /* stack: obj key mt table table key */
                        lua_insert(
                            L, -2); /*  stack: obj key mt table key table */
...
static int do_operator(lua_State *L, const char *op) {
    ...
    if (strcmp(op, ".eq") == 0) {
        ...
        tolua_error(
            L, "Attempt to perform operation on an invalid operand", NULL);
        return 0;
    }
}

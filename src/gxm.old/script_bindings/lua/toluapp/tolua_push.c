    ...
    if (value == NULL)
        ...
        if (lua_isnil(L, -1)) {
            ...
            *(void **)lua_newuserdata(
                L, sizeof(void *)) = value; /* stack: ubox u newud */
            ...
            /* check the need of updating the metatable to a more specialized
             * class */
            ...
            if (lua_istable(L, -1)) {
                lua_pushstring(
                    L, type);      /* stack: ubox[u] super super[mt] type */
            ...
            tolua_getmetatable(
                L, type); /* stack: ubox[u] super super[mt] flag mt */
...
TOLUA_API void tolua_pushfieldnumber(
    lua_State *L,
    int        lo,
    int        index,
    double     v) {

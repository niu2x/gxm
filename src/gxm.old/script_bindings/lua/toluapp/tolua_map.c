...
/* Map super classes
 * It sets 'name' as being also a 'base', mapping all super classes of 'base' in
 * 'name'
 */
...
/* Map inheritance
 * It sets 'name' as derived from 'base' by setting 'base' as metatable of
 * 'name'
 */
...
/* Register a usertype
 * It creates the correspoding metatable in the registry, for both 'type' and
 * 'const type'. It maps 'const type' as being also a 'type'
 */
...
TOLUA_API void tolua_module(lua_State *L, const char *name, int hasvar) {
    if (name) {
        ...
        tolua_push_globals_table(L);
    if (hasvar) {
        if (!tolua_ismodulemetatable(
                L)) /* check if it already has a module metatable */
        {
            ...
            if (lua_getmetatable(L, -2))
                lua_setmetatable(
                    L, -2); /* set old metatable as metatable of metatable */
            lua_setmetatable(L, -2);
        }
    }
